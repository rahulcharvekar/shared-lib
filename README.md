...
Enable `shared-lib.entity-audit.enabled=true` in your service to activate the entity audit module.

#### Zero-code Entity Auditing

To keep services lean, annotate your JPA entity with `@EntityAuditEnabled` and implement `EntityAuditDescriptor`. The shared listener snapshots the entity on load/persist/update/delete and emits audits automatically—no service-layer helper calls required.
...
Optional overrides on the descriptor let you supply metadata or change summaries. Record numbers and audit numbers continue to be generated by the shared library, and request/user context is handled the same way as manual helper calls.

> The base class does not declare an `@Id`. Define the identifier (and generator strategy) that suits your service—e.g., `IDENTITY` for MySQL, a UUID generator, etc. The listener resolves `entityId()` by calling your entity’s `getId()`.

#### Manual Helper (when needed)

You can still inject `EntityAuditHelper` for fine-grained scenarios or non-JPA flows. Besides the full `recordChange` method, you can call `recordValueChange` for the common “single-field before/after” case—the helper resolves the logged-in user and wraps the values for you:
